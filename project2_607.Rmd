---
title: "US Efficiency of Hospital-Based Emergency Care Evaluation"
author: "Chunhui Zhu"
date: "October 4, 2017"
output:
...
My method is to use left_jion function to connect these three tables. In table ndf,I observed 'measure ID' in ndf and sdf tables has some oberservations that are not in hdf, like min, max, high_min, low_min, high_max and ect.For example, "ED_1b_MEDIUM_MIN"" gives "ED_1b" lower bound of medium at the same measure. I use distinct function to remove the duplicate rows if it exists after they join. After joined three table, these observations will have a lot of NA value under hospital's variables.  
...
distinct(sumdf) #remove duplicate rows if it's exist.
dim(sumdf)
glimpse(sumdf)
```
...
The following I am going to observe if "Score.Hospital" and "Sample.Hospital" have correlation in verall, and in each category by state. 
...
From above statistic results, "Score.Hospital" and "Sample.Hospital" do not have correlation in verall, and in each category by state.
